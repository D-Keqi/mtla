...
expdir=exp #_hf           # Directory to save experiments.
...
inference_asr_model=valid.cer_ctc.best.pth # ASR model path for decoding.
                                      # e.g.
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ] && ! [[ " ${skip_stages} " =~ [[:space:]]3[[:space:]] ]]; then
    ...
    if [ "${feats_type}" = raw ]; then
        ...
        for dset in ${_dsets}; do
            ...
            scripts/audio/format_wav_scp.sh --nj "${nj}" \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ] && ! [[ " ${skip_stages} " =~ [[:space:]]5[[:space:]] ]]; then
    if [ "${token_type}" = bpe ]; then
        ...
        ./../tools/sentencepiece_commands/spm_train \
