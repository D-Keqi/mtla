...
class S2TTransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_decoder(cls, args, task, embed_tokens):
        if args.decoder_type == "roformer":
            return RotaryDecoderOnly(
                args,
                task.target_dictionary,
                embed_tokens,
                getattr(args, "no_cross_attention", False),
            )
        elif args.decoder_type == "MLA":
            return MLADecoderOnly(
                args,
                task.target_dictionary,
                embed_tokens,
                getattr(args, "no_cross_attention", False),
            )
        elif args.decoder_type == "MTLA":
            return MTLADecoderOnly(
                args,
                task.target_dictionary,
                embed_tokens,
                getattr(args, "no_cross_attention", False),
    ...
    def build_model(cls, args, task):
        ...
        if args.use_audio_input:
            ...
            model.frontend.upstream.eval()
            for param in model.frontend.upstream.parameters():
                param.requires_grad_(False)

            model.specaug = SpecAug(
                freq_mask_width_range=(0, 30), time_mask_width_range=(0, 40)
            )
            model.normalize = UtteranceMVN()
            model.preencoder = LinearProjection(1024, 80)
            model.new_norm = torch.nn.LayerNorm(80)
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens):
        ...
        if self.frontend is not None:
            ...
            with torch.cuda.amp.autocast(enabled=False):
                src_tokens, src_lengths = self.frontend(src_tokens, src_lengths)

                # 2. Data augmentation
                if self.specaug is not None and self.training:
                    src_tokens, src_lengths = self.specaug(src_tokens, src_lengths)

                # 3. Normalization for feature: e.g. Global-CMVN, Utterance-CMVN
                if self.normalize is not None:
                    src_tokens, src_lengths = self.normalize(src_tokens, src_lengths)

            # Pre-encoder, e.g. used for raw input data
            # if self.preencoder is not None:
            src_tokens, src_lengths = self.preencoder(src_tokens, src_lengths)
            src_tokens = self.new_norm(src_tokens)
    ...
    def forward_encoder_torchscript(self, net_input: Dict[str, Tensor]):
        ...
        This is used to pre-extract the speech features
        """
...
@register_model_architecture("s2t_transformer", "s2t_decoder_only_roformer_cross_xm")
def s2t_decoder_only_roformer_cross_xm(args):
    args.decoder_type = getattr(args, "decoder_type", "roformer")
    ...
    args.encoder_projector_ds_rate = getattr(args, "encoder_projector_ds_rate", 1)
    base_architecture(args)


@register_model_architecture("s2t_transformer", "s2t_decoder_only_GQAroformer_cross_xm")
def s2t_decoder_only_GQAroformer_cross_xm(args):
    args.decoder_type = getattr(args, "decoder_type", "roformer")
    args.decoder_att_type = getattr(args, "decoder_att_type", "GQA")
    args.decoder_layers = getattr(args, "decoder_layers", 9)
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 512 * 4)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 8)
...
@register_model_architecture("s2t_transformer", "s2t_decoder_only_MQAroformer_cross_xm")
def s2t_decoder_only_MQAroformer_cross_xm(args):
    args.decoder_type = getattr(args, "decoder_type", "roformer")
    args.decoder_att_type = getattr(args, "decoder_att_type", "MQA")
    args.decoder_layers = getattr(args, "decoder_layers", 9)
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 512 * 4)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 8)
...
@register_model_architecture("s2t_transformer", "s2t_decoder_only_MLA_cross_xm")
def s2t_decoder_only_MLA_xm(args):
    args.decoder_type = getattr(args, "decoder_type", "MLA")
    args.decoder_layers = getattr(args, "decoder_layers", 9)
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 512 * 4)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 8)
    ...
    args.decoder_attention_heads = getattr(args, "decoder_attention_heads", 8)
    args.decoder_q_lora_rank = getattr(args, "decoder_q_lora_rank", 0)
    args.decoder_kv_lora_rank = getattr(args, "decoder_kv_lora_rank", 256)
    args.decoder_qk_nope_head_dim = getattr(args, "decoder_qk_nope_head_dim", 64)
    args.decoder_qk_rope_head_dim = getattr(args, "decoder_qk_rope_head_dim", 32)
    args.decoder_v_head_dim = getattr(args, "decoder_v_head_dim", 64)
    args.dropout = getattr(args, "dropout", 0.15)
    args.no_cross_attention = getattr(args, "no_cross_attention", True)
    args.encoder_projector_ds_rate = getattr(args, "encoder_projector_ds_rate", 1)
    base_architecture(args)
...
@register_model_architecture("s2t_transformer", "s2t_decoder_only_MTLA_cross_xm")
def s2t_decoder_only_MTLA_xm(args):
    args.decoder_type = getattr(args, "decoder_type", "MTLA")
    args.decoder_layers = getattr(args, "decoder_layers", 9)
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 512 * 4)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 8)
    ...
    args.decoder_qk_rope_head_dim = getattr(args, "decoder_qk_rope_head_dim", 32)
    args.decoder_v_head_dim = getattr(args, "decoder_v_head_dim", 64)
    args.decoder_down_rate = getattr(args, "decoder_down_rate", 2)
    args.dropout = getattr(args, "dropout", 0.15)
    args.no_cross_attention = getattr(args, "no_cross_attention", True)
    args.encoder_projector_ds_rate = getattr(args, "encoder_projector_ds_rate", 1)
    base_architecture(args)
