...
class S2TConformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args):
        ...
        if getattr(args, "ctc_weight", 0.0) > 0.0:
            self.ctc_proj = torch.nn.Linear(args.encoder_embed_dim, args.tgt_dict_size)
...
@register_model_architecture("s2t_conformer", "conformer_decoder_only_MTLA_cross_xm")
def conformer_decoder_only_MTLA_xm(args):
...
@register_model_architecture("s2t_conformer", "conformer_decoder_only_MLA_cross_xm")
def conformer_decoder_only_MLA_xm(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 2048)
    args.encoder_layers = getattr(args, "encoder_layers", 12)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 8)
...
@register_model_architecture(
    "s2t_conformer", "conformer_decoder_only_roformer_cross_xm"
)
def conformer_decoder_only_roformer_cross_xm(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 2048)
    args.encoder_layers = getattr(args, "encoder_layers", 12)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 8)
