...
def load_kaldi(input, need_waveform=False):
    ...
    if need_waveform:
        return array
    return extract_fbank(array)


def extract_fbank(waveform, sample_rate=16000):
    """
    Extract FBANK features from a NumPy array or PyTorch Tensor

    Parameters:
        waveform: Input audio, supports NumPy array or PyTorch Tensor of shape (samples,) or (1, samples)
        sample_rate: Audio sampling rate (default: 16000)

    Returns:
        fbank: FBANK feature matrix of shape (T, 80)
    """
    ...
    # Reshape to ensure shape is (1, samples)
    if waveform.dim() == 1:
...
def get_features_or_waveform(
    ...
    elif len(slice_ptr) == 1:
        features_or_waveform = load_kaldi(path, need_waveform)
