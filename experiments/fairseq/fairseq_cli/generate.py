...
def _main(cfg: DictConfig, output_file):
    ...
    for model in models:
        model.encoder.embed_positions.weights = model.encoder.embed_positions.weights[
            :6000
        ]
    ...
    b_model_mem = torch.cuda.memory_allocated()
    ...
    a_model_mem = torch.cuda.memory_allocated()
    ...
    print(f"Before Model-only memory: {b_model_mem / 1024**2:.2f} MiB")
    print(f"After Model-only memory: {a_model_mem / 1024**2:.2f} MiB")
    return scorer
...
def monitor_memory():
    """Background thread: periodically logs GPU memory usage"""
...
def cli_main():
    ...
    # Compuate GPU memory usage
    peak_mem = torch.cuda.max_memory_allocated() / 1024**2  # MiB
    avg_mem = sum(memory_samples) / len(memory_samples) / 1024**2  # MiB
