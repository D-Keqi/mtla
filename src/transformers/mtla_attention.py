...
class MultiheadTemporalLatentAttentionHF(nn.Module):
    """
    HF-compatible wrapper of Multihead Temporal Latent Attention.
    ...
    def forward(
        ...
        **kwargs,
        ...
        if (
            ...
            # logging.warning(f"!!!{q_pe_t.shape}, {k_pe_rep.shape}, {cos.shape}, {sin.shape}")
        ...
        if use_cache and past_key_value is not None:
            # Detect if first token
            is_first_step = past_key_value.is_empty(self.layer_idx)
            T = (
                cache_position.max().item() + 1
            ...
            if is_first_step:
                # Case A: Only one token (pure first step)
                if seqlen == 1:
                    kv_norm_t = kv_norm * w_tT
                    prev_kv_t, prev_k_pe = past_key_value.update(
                        kv_norm_t, k_pe, self.layer_idx, seqlen
                    )

                # Case B: prompt tokens exist
                else:
                    if self.recompute_prompt_attn:
                        ...
                        prev_kv_t = kv_norm_t
                        prev_k_pe = k_pe
                        kv_t, k_pe_t = past_key_value.update(
                            prev_kv_t[:, indices],
                            prev_k_pe[:, indices],
                            self.layer_idx,
                            seqlen,
                        ...
                        kv_norm_t = torch.matmul(
                            w_tT * zero_mask[indices].unsqueeze(0), kv_norm
                        )
                        k_pe = k_pe[:, indices]
                        prev_kv_t, prev_k_pe = past_key_value.update(
                            kv_norm_t, k_pe, self.layer_idx, seqlen
                ...
                # Case C: normal incremental update
                kv_norm_t = kv_norm * w_tT
                prev_kv_t, prev_k_pe = past_key_value.update(
                    kv_norm_t, k_pe, self.layer_idx, 1
                )
                tricky_mask = None
            ...
            if attention_mask is not None:
                if is_first_step and self.recompute_prompt_attn:
                    attention_mask = (1 - attention_mask) * -1e9
                else:
                    attention_mask = (1 - attention_mask[:, indices]) * -1e9
            ...
            x = torch.einsum("bsht,btc->bshc", attn, prev_kv_t)
