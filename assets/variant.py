...
class MultiheadTemporalLatentCrossAttention(nn.Module):
    """
    Multi-head Temporal Latent Attention (MTLA) is not designed for cross-attention; this implementation merely provides a possibility.
    ...
    def __init__(
        ...
        if cross_att:
            # Cross attention does not need RoPE
            qk_rope_head_dim = 0
    ...
    def forward(
        ...
        if incremental_state is not None:
            ...
            t = math.ceil(T / self.down_rate)
            w_tT = self.hypernet_down(
                T, t, kv_norm.device, train=not self.cross_att, T_input=kv_norm
            ...
            if self.cross_att:
                ...
                zero_mask = zero_mask[
                    indices
                ]  # Do not need to simulate the incremental inference case in cross attention
            zero_mask = zero_mask.unsqueeze(0).to(kv_norm.dtype)
